...
State state_transition(const State& before, const vector<long long>& character) {
	...
	bs c_bitset_transition = bitset_transition(before.c, character);
	bs cr_bitset_transition = rev_bitset_transition(before.cr, character);
	addEpsilonTrans(a_bitset_transition, b_bitset_transition, br_bitset_transition, c_bitset_transition, cr_bitset_transition);
	return State(a_bitset_transition, b_bitset_transition, br_bitset_transition, c_bitset_transition, cr_bitset_transition);
}

bool isFinalState(const State& state) {
	return state.c[N-1] || state.cr[0];
}
...
int main() {
	...
	while (!s.empty()) {
		...
		for (const vector<long long>& character: alphabet) {
			if (isValidTransition(currState.br, character) and isValidTransition(currState.cr, character)) {
	...
	while (true) {
		...
		for (auto& pair : stateToGroup) {
			...
			for (const vector<long long>& character : alphabet) {
				if (isValidTransition(pair.first.br, character) and isValidTransition(pair.first.cr, character)) {
